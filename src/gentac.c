...
TOKEN* peep_dest(){
    TOKEN* peeped = peep();
    if(peeped==NULL){printf("fatal: failed to get destination\n");exit(1);}
    return peeped;
}
    ...
    if (isgraph(c) || c==' ') {
        switch(c){
            ...
            case '+':
                ...
                last = find_last(last);
                new_dest(env);
                last->next = new_stac(tac_plus,left,right,get_dest());
                return tac;
            case '-':
                new_dest(env);
                tac = gen_tac0(tree->left,env);
                ...
                last = find_last(last);
                new_dest(env);
                last->next = new_stac(tac_minus,left,right,get_dest());
                return tac;
            case '*':
                new_dest(env);
                tac = gen_tac0(tree->left,env);
                ...
                last = find_last(last);
                new_dest(env);
                last->next = new_stac(tac_mult,left,right,get_dest());
                return tac;
            case '/':
                new_dest(env);
                tac = gen_tac0(tree->left,env);
                ...
                last = find_last(last);
                new_dest(env);
                last->next = new_stac(tac_div,left,right,get_dest());
                return tac;
            case '%':
                new_dest(env);
                tac = gen_tac0(tree->left,env);
                ...
                last = find_last(last);
                new_dest(env);
                last->next = new_stac(tac_mod,left,right,get_dest());
                return tac;
        }
    }
    switch(tree->type){
